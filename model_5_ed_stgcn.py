...
class TPPblock(nn.Module):
    ...
    def forward(self, x):
        ...
        '''x1_gcn = self.gcn(x_pool1, A)
        x2_gcn = self.gcn(x_pool2, A)
        x3_gcn = self.gcn(x_pool3, A)
        x4_gcn = self.gcn(x_pool4, A)'''
...
class down(nn.Module):
    ...
    def forward(self, x, A, MaxPool):
        ...
        # reshape the input tensor to match the output tensor dimensions
        #res = nn.functional.interpolate(res, (x.size(2), x.size(3)), mode='bilinear', align_corners=False)
