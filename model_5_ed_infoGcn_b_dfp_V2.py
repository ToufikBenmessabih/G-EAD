...
def get_adj(s, A):
    ...
    # Learning Adj(L=1)
    # S transpose
    st = s.transpose(2,3) # Shape: [[B, T, 21, 5]
    # calculate the new Adjacency A(l+1) = S*A*St
    ''' 
    S = [5, 460, 5, 21]
    Adj [3, 21, 21]
    ST = [5, 460, 21, 5] 
    '''
    adj = torch.einsum('ntcv,kvw->kcw', (s, A)) # Shape: [[3, 5, 21]
    adj = torch.einsum('kcw, ntwv->kcv', (adj, st)) # Shape: [[3, 5, 5]
    #print('adj (l+1)): ', adj.shape)
...
def get_x(z, s, c):
    ...
    # Permute dimensions to bring 460 to the front
    s =s.permute(0, 2, 1, 3)  # Shape: [5, 460, 5, 21]
    # Reshape to combine the batch and temporal dimensions
    s = s.reshape(-1, c, v1)  # Shape: [5*460, 5, 21]
    # Apply softmax along the last dimension
    s = torch.softmax(s, dim=-1)  # Shape: [B, 5, 21]
    #reshape for multiplication
    nt, d, v2 = s.size()
    s = s.view(nt//t, t, d, v2) # Shape: [B, T, 5, 21]  
    z =z.permute(0, 2, 3, 1)  # Shape: [5, 460, 21, 64]  ZT

    # Perform batch matrix multiplication
    x = torch.matmul(s, z)  # Shape: [5, 460, 5, 64]
    x = x.permute(0,3,1,2)
    #print('x (l+1)): ', x.shape)
...
class outconv(nn.Module):
    def __init__(self, in_ch, nbr_classes):
        super(outconv, self).__init__()
        # clusters: 5, 7

        self.gcn_embed = ConvGraphical(in_ch, 128, 3, 1)
        self.gcn_pool = ConvGraphical(in_ch, 21 , 3, 1)
        self.gcn_embed_2 = ConvGraphical(128, nbr_classes, 3, 1)
        self.gcn_pool_2 = ConvGraphical(128, 13 , 3, 1)
        self.relu = nn.ReLU()

    def forward(self, x, A): # ----------------------------------------------------------
        
        '''
        x(0)):   torch.Size([5, 64, 460, 21])
        adj(0)): torch.Size([3, 21, 21])
        '''
        # learning X(L+1) and Adj(L+1)
        z = self.relu(self.gcn_embed(x, A))
        s = self.relu(self.gcn_pool(x, A))

        x, z, s= get_x(z, s, 21)
        A, st= get_adj(s, A)
        
        # learning X(L+1) and Adj(L+1)
        z = self.relu(self.gcn_embed_2(x, A))
        s = self.relu(self.gcn_pool_2(x, A))

        x, z, s= get_x(z, s, 13)

        #avg pool
        x = F.avg_pool2d(x, kernel_size=(1, x.size(-1)))
        x = torch.mean(x, dim=-1)  # torch.Size([5, 14, 460])
...
class TPPblock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size):
        ...
        self.gcn = InfoGCN(in_channels+4, in_channels+4, kernel_size[1])
