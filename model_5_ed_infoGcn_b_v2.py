...
class TPPblock(nn.Module):
    ...
    def forward(self, x, A):
        ...
        # Reshape to merge the last two dimensions
        N, C, V, L = x_permuted.shape
        x_reshaped = x_permuted.view(N * C, V, L)  # Shape: [1280, 21, 230]
        #print('x_reshaped: ', x_reshaped.shape)
        ...
        x_pooled1_reshaped = x_pool1.view(N, C, M_pooled, L_pooled)  # Shape: [5, 128, 21, 115]
        ...
        # Permute back to the original shape
        x_pool1 = x_pooled1_reshaped.permute(0, 1, 3, 2)
        ...
        #print('x_pool1 after reshape: ', x_pool1.shape)  # [5, 128, 115, 21]
        ...
        #print('x1_conv after conv: ', x1_conv.shape) # [5, c=32, 115, 21]

        # reshape to 3D to upsample
        x1_conv = x1_conv.view(-1, x1_conv.shape[2], x1_conv.shape[3])
        x2_conv = x2_conv.view(-1, x2_conv.shape[2], x2_conv.shape[3])
        x3_conv = x3_conv.view(-1, x3_conv.shape[2], x3_conv.shape[3])
        x4_conv = x4_conv.view(-1, x4_conv.shape[2], x4_conv.shape[3])
        #print('x1_conv after reshape: ', x1_conv.shape) # [5*32, 115, 21]

        # permute and apply upsampling on last dim
        x1_conv = x1_conv.permute(0, 2, 1)  
        x2_conv = x2_conv.permute(0, 2, 1) 
        x3_conv = x3_conv.permute(0, 2, 1) 
        x4_conv = x4_conv.permute(0, 2, 1)  # Shape: [5*32, 21, 115]
        ...
        # permute back
        x_up1 = self.layer1.permute(0, 2, 1)
        x_up2 = self.layer2.permute(0, 2, 1)
        x_up3 = self.layer3.permute(0, 2, 1)
        x_up4 = self.layer4.permute(0, 2, 1)
        #print('x1 upsampled: ', x_up1.shape) # [5*32, 230, 21]

        # reshape to origin
        x_up1 = x_up1.view(N, x_up1.size(0) // N, x_up1.size(1), V)
        x_up2 = x_up2.view(N, x_up2.size(0) // N, x_up2.size(1), V)
        x_up3 = x_up3.view(N, x_up3.size(0) // N, x_up3.size(1), V)
        x_up4 = x_up4.view(N, x_up4.size(0) // N, x_up4.size(1), V)
        #print('x1 final: ', x_up1.shape)  # [5, 32, 230, 21]
        
        out = torch.cat([x_up1, x_up2, x_up3, x_up4, x], 1)
        #print('out befor att: ', out.shape) # [5, 256, 230, 21]

        out = self.gcn(out, A)
        #print('out after att: ', out.shape)
