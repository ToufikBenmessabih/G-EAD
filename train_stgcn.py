...
config = dotdict(
    ...
    #feature_size =  216, #inhard_13
    #feature_size =  63, #inhard_13
    feature_size =  99, #IKEA 3D
...
#config.features_file_name = config.base_dir + "/features/inhard-13/30fps_p"  #inhard
config.features_file_name = config.base_dir + "/features/25fps_p_3D"   #ikea
...
config.ground_truth_files_dir = config.base_dir + "/groundTruth/verbs/"  #ikea
#config.ground_truth_files_dir = config.base_dir + "/groundTruth/bvh_30fps/"  #inhard
config.label_id_csv = config.base_dir + 'mapping_verb.csv'
...
class CriterionClass(nn.Module):
    def __init__(self, config):
        ...
        class_weights = torch.tensor([40.0, 51.0, 466.0, 38.0, 7338.0, 35.0, 1030.0, 84.0, 1927.0, 33.0, 25.0, 1.0, 14.0, 3.0]) #14 verbs
...
def train(model, loader, criterion, optimizer, scheduler, config, test_loader, postprocessor):
    ...
    for epoch in range(config.epochs):
        ...
        for i, item in enumerate(loader):
            ...
            outputs_ensemble = get_c2f_ensemble_output(outputs, config.ensem_weights)
